...
%% Listen to Audio
% p8 = audioplayer(dp(:,1),Fs); playblocking(p8);

%% Build Spec
pinkf_spec = band_spec(pinkf,rate);
bs_spec = band_spec(bs,rate);
dp_spec = band_spec(dp,rate);

clearvars pinkf bs dp

%% Training Set
feature = 20;
[r1,r2,r3,W,U,S,V,mv,eq,center] = band_trainer(pinkf_spec,bs_spec,dp_spec,feature);

% clearvars pinkf_spec bs_spec dp_spec
...
testmat = U'*test_spec;
pval = W'*testmat;
labels = [ones(1,size(pinkf,2)),ones(1,size(bs,2))+1,ones(1,size(dp,2))+2];
...
plot(pval(1,1:size(pinkf,2)),pval(2,1:size(pinkf,2)),'k+','MarkerSize',10,'LineWidth',2)
plot(pval(1,size(pinkf,2)+1:size(pinkf,2)+size(bs,2)),pval(2,size(pinkf,2)+1:size(pinkf,2)+size(bs,2)),'r+','MarkerSize',10,'LineWidth',2)
plot(pval(1,size(pinkf,2)+size(bs,2)+1:size(pinkf,2)+size(bs,2)+size(dp,2)),pval(2,size(pinkf,2)+size(bs,2)+1:size(pinkf,2)+size(bs,2)+size(dp,2)),'g+','MarkerSize',10,'LineWidth',2)
