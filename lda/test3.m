...
num_songs = 9;
pinkf = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/pinkf', num2str(2*i), '.m4a'];
    pinkf(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

num_songs = 8;
bs = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/bs', num2str(2*i), '.mp3'];
    bs(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

num_songs = 11;
dp = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/dp', num2str(2*i), '.mp3'];
    dp(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

rock = [pinkf,bs,dp];

num_songs = 10;
bg = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['disco/bg', num2str(2*i), '.mp3'];
    bg(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

num_songs = 9;
psb = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['disco/psb', num2str(2*i), '.mp3'];
    psb(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

disco = [bg,psb];

num_songs = 5;
ej = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['jazz/ej', num2str(2*i), '.mp3'];
    ej(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

num_songs = 10;
miles = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['jazz/miles', num2str(2*i), '.mp3'];
    miles(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

num_songs = 8;
tony = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['jazz/tony', num2str(2*i), '.mp3'];
    tony(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end

jazz = [ej,miles,tony];

%% Listen to Audio
% p8 = audioplayer(disco(:,1),Fs); playblocking(p8);

%% Build Spec
rock_spec = band_spec(rock,rate);
disco_spec = band_spec(disco,rate);
jazz_spec = band_spec(jazz,rate);

clearvars rock jazz disco

%% Training Set
feature = 15;
[r1,r2,r3,W,U,S,V,mv,eq,center] = band_trainer(rock_spec,jazz_spec,disco_spec,feature);

% clearvars rock_spec jazz_spec disco_spec
...
pinkf = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/pinkf', num2str(2*i-1), '.m4a'];
    pinkf(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
bs = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/bs', num2str(2*i-1), '.mp3'];
    bs(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
dp = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['rock/dp', num2str(2*i-1), '.mp3'];
    dp(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
bg = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['disco/bg', num2str(2*i-1), '.mp3'];
    bg(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
psb = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['disco/psb', num2str(2*i-1), '.mp3'];
    psb(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
ej = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['jazz/ej', num2str(2*i-1), '.mp3'];
    ej(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
tony = zeros(rate*sec_len,num_songs*sps);
for i = 1:num_songs
    path = ['jazz/tony', num2str(2*i-1), '.mp3'];
    tony(:,i*sps-(sps-1):i*sps) = adj_music(path,sec_len,start_pt,sps,rate,num_samples);
end
...
%
test = [rock,jazz,disco];
test_spec = band_spec(test,rate);
testmat = U'*test_spec;
pval = W'*testmat;
labels = [ones(1,size(rock,2)),ones(1,size(jazz,2))+1,ones(1,size(disco,2))+2];
...
plot(pval(1,size(rock,2)+size(jazz,2)+1:size(rock,2)+size(jazz,2)+size(disco,2)),pval(2,size(rock,2)+size(jazz,2)+1:size(rock,2)+size(jazz,2)+size(disco,2)),'g+','MarkerSize',10,'LineWidth',2)
